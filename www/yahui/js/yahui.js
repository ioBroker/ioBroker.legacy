...
$(document).ready(function () {
    ...
    yahui.socket.on('error', function (err) {
    ...
    function renderMenuItem(enId, linkclass) {
        ...
        //console.log('renderMenuItem', enId, enObj);
        if (!enObj || !enObj.Name) {
            return "";
        }
        ...
        if (linkclass) {
            if ("#page_"+enId == url.hash.replace(/\\\./g, '.')) {
        ...
        return "<li class='"+liClass+"' data-hm-id='"+enId+"'><div data-hm-service-msg='"+enId+"' style='" + (serviceMsgCount==0 ? "display:none" : "") + "' class='service-message'><div class='service-message-count'>" + (serviceMsgCount == 0 ? "" : serviceMsgCount) + "</div></div><a href='#page_"+enId.replace(/\./g, '\\.')+"'" + (linkclass && linkclass != "" ? " class='" + linkclass + "'" : "") + ">" +
            "<img src='"+img+"'>" +
            "<h2>"+enObj.Name+ "</h2>"+
    ...
    $(document).bind( "pagebeforechange", function( e, data ) {
        ...
        if ( typeof data.toPage === "string" ) {
            ...
            if ( u.hash.search(/^#page_/) !== -1 ) {
                ...
                if (!$("div[id='page_"+pageId+"']").html()) {
                    renderPage(pageId);
                    //$.mobile.changePage('#page_' + pageId.replace(/\./g, '\\\\.'));
                    console.log('---');
                }
            // Erweiterungen
    ...
    function renderIFrame(pageId) {
        ...
        body.prepend(page);
        $("[id='refresh_"+pageId+"']").click(function () {
           $("[id='if_"+pageId+"']").attr('src', function ( i, val ) { return val; });
        });
    }
    ...
    function renderPage(pageId, prepend) {
        ...
        if (regaObj.TypeName == "ENUM_ROOMS" || regaObj.TypeName == "ENUM_FUNCTIONS" || regaObj.TypeName == "FAVORITE") {
            ...
            $("ul[id='ul_list_left_" + pageId + "']").empty();
            $("ul[id='ul_list_left_" + pageId + "']").append('<li id="heading_' + myid + '_' + pageId + '" data-role="list-divider" role="heading">' + name + '</li>');
            $("ul[id='ul_list_left_collapsed_" + pageId + "']").empty();
            $("ul[id='ul_list_left_collapsed_" + pageId + "']").append('<li id="heading_collapsed_' + myid + '_' + pageId + '" data-role="list-divider" role="heading">' + name + '</li>');
            AnimateRotate("ul[id='ul_list_left_collapsed_" + pageId + "']", 90);
            ...
            $('[id="heading_' + myid + '_' + pageId + '"]').on("click", function() {
            ...
            $('[id="heading_collapsed_' + myid + '_' + pageId + '"]').on("click", function() {
            ...
            renderMenu(regaObj.TypeName, "ul[id='ul_list_left_" + pageId + "']", "nopadding");
        }

        var list = $("ul[id='list_"+pageId + "']");
    ...
    function renderAlarmWidget(list, id, varEdit, pageId, cb) {
        ...
        $("[id='alarm_"+id+"']").click(function (e) {
            yahui.socket.emit("alarmReceipt", event.target.dataset.hmId);
            $(this).remove();
        });
    ...
    function renderWidget(list, id, varEdit, pageId) {
        ...
        console.log("renderWidget", list, id, el, pageId);
...
/*
            ...
            switch (el.HssType) {
                ...
                case "HUE_DIMMABLE_PLUG-IN_UNIT":
                    ...
                    setTimeout(function () {
                        $("[id^='slider_"+elId+"_']").on( 'slidestop', function( event ) {
                            yahui.socket.emit("setState", [event.target.dataset.hmId, parseInt(event.target.value,10)]);
                        });
                        $("[id^='hueswitch_"+elId+"_']").on( 'slidestop', function( event ) {
                            yahui.socket.emit("setState", [event.target.dataset.hmId, (event.target.value == "true" ? true : false)]);
                        });
                    }, 500);
                    break;
                case "HUE_COLOR_LIGHT":
                    ...
                    content += '</div><div class="yahui-c">' +
                        ...
                        '" name="slider_'+elId+'_SAT" min="0" max="255" value="'+(datapoints[satId][0])+'"/><br/>' +
                        '<div class="yahui-slider-hue">Farbe'+
                        '<input id="slider_'+elId+'_HUE" type="range" data-hm-factor="1" data-hm-id="'+hueId +
                        '" name="slider_'+elId+'_HUE"  min="0" max="65535" value="'+(datapoints[hueId][0])+'"/></div></div></li>';
                    list.append(content);
                    setTimeout(function () {
                        $("[id^='slider_"+elId+"_']").on( 'slidestop', function( event ) {
                ...
                case "HUE_EXTENDED_COLOR_LIGHT":
                    ...
                    setTimeout(function () {
                        ...
                        $("[id^='hueswitch_"+elId+"_']").on( 'slidestop', function( event ) {
                            yahui.socket.emit("setState", [event.target.dataset.hmId, event.target.value]);
                ...
                case "DIMMER":
                    ...
                    content = '<li class="yahui-widget ' + visibleClass + '" data-hm-id="'+id+'"><img src="'+img+'" alt="" data-hm-id="' + id + '" />' +
                        ...
                        '<option value="100"'+((datapoints[levelId][0] > 0) ?' selected':'')+'>An</option>' +
                        '</select>'+lowbat;
                    ...
                    console.log('dimmer', levelId, datapoints[levelId]);
                    content += '</div><div class="yahui-c" data-hm-id="' + id + '">' +
                        '<input id="'+elId+'" type="range" data-hm-factor="1" data-hm-id="'+levelId +
                        '" name="slider_'+elId+'" id="slider_'+elId+'" min="0" max="100" value="'+(datapoints[levelId][0])+'"/></div></li>';
                    list.append(content);
                    setTimeout(function () {
                        $("[id='"+elId+"']").on( 'slidestop', function( event ) {
                            ...
                            yahui.socket.emit("setState", [event.target.dataset.hmId, parseInt(event.target.value,10) / (event.target.dataset.hmFactor?event.target.dataset.hmFactor:1)]);
                        });
                        $("[id='switch_"+elId+"']").on( 'slidestop', function( event ) {
                ...
                case "BLIND":
                    ...
                    content += '</div><div class="yahui-c" data-hm-id="' + id + '">' +
                        '<input type="range" data-hm-factor="1" data-hm-id="'+levelId +
                        '" name="slider-1" id="'+elId+'" min="0" max="100" value="'+(datapoints[levelId][0])+'"/></div></li>';
                    ...
                    setTimeout(function () {
                        $("[id='"+elId+"']").on( 'slidestop', function( event ) {
                            //console.log("slide "+event.target.value / (event.target.dataset.hmFactor?event.target.dataset.hmFactor:1)+" "+event.target.dataset.hmId);
                            yahui.socket.emit("setState", [event.target.dataset.hmId, event.target.value / (event.target.dataset.hmFactor?event.target.dataset.hmFactor:1)]);
                        });
                        $("[id='switch_"+elId+"']").on( 'slidestop', function( event ) {
                            //console.log("slide "+event.target.value+" "+event.target.dataset.hmId);
                            yahui.socket.emit("setState", [event.target.dataset.hmId, parseInt(event.target.value,10)]);
                ...
                case "KEY":
                    ...
                    if (el.Parent && regaObjects[el.Parent] && regaObjects[el.Parent].HssType == "HM-OU-LED16") {
                        ...
                        content = '<li class="yahui-widget ' + visibleClass + '" data-hm-id="'+id+'"><img src="'+img+'" alt="" data-hm-id="' + id + '" />' +
                            ...
                            '</div></li>';
                        list.append(content);
                        $("[id='press_short_"+elId+"']").click(function (e) {
                            //console.log("press short "+id);
                            yahui.socket.emit("setState", [event.target.dataset.hmId, true]);
                        });
                        break;
                    }
                case "VIRTUAL_KEY":
                    ...
                    if (!settings.hideKeys) {
                        ...
                        $("[id='press_long_"+elId+"']").click(function (e) {
                ...
                case "KEYMATIC":
                    ...
                    setTimeout(function () {
                        ...
                        $("[id='open_"+elId+"']").click(function (event) {
                            //console.log("press short "+id);
                            yahui.socket.emit("setState", [event.target.dataset.hmId, true]);
                        });
                    }, 500);
                ...
                case "CLIMATECONTROL_RT_TRANSCEIVER":
                    ...
                    setTimeout(function () {
                        $("[id='input_"+id+"']").change(function( event ) {
                            var val = $("[id='input_"+id+"']").val();
                            var mode = parseInt($("[id='select_"+elId+"'] option:selected").val(), 10);
                            if (mode == 1) {
                        ...
                        $("[id='select_"+elId+"']").on( 'change', function( event ) {
                            var val = parseInt($("[id='select_"+elId+"'] option:selected").val(), 10);
                            var setTemperature = $("[id=input_"+id+"']").val();
                            switch (val) {
                                case 0:
                    ...
                    case "16-29": // Werteliste
                        ...
                        setTimeout(function () {
                            $("[id='select_"+elId+"']").on( 'change', function( event ) {
                                ...
                                yahui.socket.emit("setState", [event.target.dataset.hmId, val]);
                            });
                        }, 500);
                        break;
    ...
    function updateWidgets(id, val, lastupdate, ack, ts) {
        ...
        if (val == 1 && regaIndex.ALDP && regaIndex.ALDP.indexOf(id) != -1) {
            if ($("ul#list_alarms").html() && !$("li.yahui-alarm[data-hm-id='"+id+"']").html()) {
                renderAlarmWidget($("ul#list_alarms"), id, false, "#alarms", function () {
        ...
        $(".hue-switch[data-hm-id='"+id+"']").each(function () {
            ...
            if (val == "ct") {
                $("[id='"+$this.attr("id").replace(/hueswitch_/,"").replace(/_COLORMODE/,"")+"_CT']").show();
                $("[id='"+$this.attr("id").replace(/hueswitch_/,"").replace(/_COLORMODE/,"")+"_HS']").hide();
            } else if (val == "hs") {
                $("[id='"+$this.attr("id").replace(/hueswitch_/,"").replace(/_COLORMODE/,"")+"_CT']").hide();
                $("[id='"+$this.attr("id").replace(/hueswitch_/,"").replace(/_COLORMODE/,"")+"_HS']").show();
            }
        });
